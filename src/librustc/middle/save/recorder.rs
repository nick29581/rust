...
impl Recorder {
    pub fn record(&mut self, info: StrBuf) {
    ...
    pub fn dump_span(&mut self,
        ...
        self.record(StrBuf::from_str(result));
    }
}
...
impl<'a> FmtStrs<'a> {
    pub fn new(rec: Box<Recorder>, span: SpanUtils<'a>, krate: StrBuf) -> FmtStrs<'a> {
        FmtStrs {
            recorder: rec,
            span: span,
            krate: krate,
    ...
    fn LookupRow(r: Row) -> (&'static str, Vec<&'static str>, bool, bool) {
        match r {
            Variable => ("variable",
    ...
    pub fn make_values_str(&self,
        ...
        let mut strs = pairs.map(|(f, v)| format!(",{},\"{}\"", f, escape(
            if *f == "qualname" {
                ...
                StrBuf::from_str(v.to_owned())
            }
        )));
        Some(strs.fold(StrBuf::new(), |s, ss| s.append(ss))).map(|s| s.into_owned())
    }

    pub fn record_without_span(&mut self,
                               kind: Row,
                               values: Vec<~str>) {
        let (label, ref fields, needs_span, dump_spans) = FmtStrs::LookupRow(kind);

        if needs_span {
            self.span.sess.bug(format!(
                "Called record_without_span for '{}' which does requires a span", label));
        }
        assert!(!dump_spans);
        ...
        let values_str = match self.make_values_str(label, fields, values) {
            Some(vs) => vs,
            None => return,
        };

        let result = StrBuf::from_str(label);
        self.recorder.record(result.append(values_str.into_owned()).append("\n"));
    }
    ...
    pub fn record_with_span(&mut self,
        ...
        if !needs_span {
            self.span.sess.span_bug(span, format!("Called record_with_span for '{}' \
        ...
        self.recorder.record(StrBuf::from_str(result));
    }
    ...
    pub fn check_and_record(&mut self,
        ...
        match sub_span {
            ...
            None => {
                let (label, _, _, _) = FmtStrs::LookupRow(kind);
    ...
    pub fn variable_str(&mut self,
        ...
        self.check_and_record(Variable,
                              span,
                              sub_span,
                              svec!(id, name, name + "$" + id.to_str(), value, typ, 0));
    }
    ...
    pub fn formal_str(&mut self,
        ...
        self.check_and_record(Variable,
                              span,
                              sub_span,
                              svec!(id, name, fn_name + "::" + name, "", typ, 0));
    }
