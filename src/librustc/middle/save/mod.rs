...
fn escape(s: StrBuf) -> StrBuf {
    StrBuf::from_str(s.replace("\"", "\"\""))
}
...
impl <'l> DxrVisitor<'l> {
    fn dump_crate_info(&mut self, name: &str, krate: &ast::Crate) {
        ...
        self.fmt.recorder.record(StrBuf::from_str("end_external_crates\n"));
    }

    // Return all non-empty prefixes of a path.
    // For each prefix, we return the span for the last segment in the prefix and
    // a str representation of the entire prefix.
    fn process_path_prefixes(&self, path: &ast::Path) -> Vec<(Span, ~str)> {
        ...
        let mut result: Vec<(Span, ~str)> = vec!();
        ...
        for (seg, span) in path.segments.iter().zip(spans.iter()) {
            ...
            result.push((*span, qualname.into_owned()));
    ...
    fn lookup_type_ref(&self, ref_id: NodeId) -> Option<DefId> {
        if !self.analysis.ty_cx.def_map.borrow().contains_key(&ref_id) {
            self.sess.bug(format!("def_map has no key for {} in lookup_type_ref", ref_id));
        }
    ...
    fn lookup_def_kind(&self, ref_id: NodeId) -> Option<recorder::Row> {
        let def_map = self.analysis.ty_cx.def_map.borrow();
        if !def_map.contains_key(&ref_id) {
            self.sess.bug(format!("def_map has no key for {} in lookup_def_kind", ref_id));
        }
        let def = *def_map.get(&ref_id);
        match def {
            ast::DefMod(_) |
            ast::DefForeignMod(_) => Some(recorder::ModRef),
            ast::DefStruct(_) => Some(recorder::StructRef),
            ast::DefTy(_) |
            ast::DefTrait(_) => Some(recorder::TypeRef),
            ast::DefStatic(_, _) |
            ast::DefBinding(_, _) |
            ast::DefArg(_, _) |
            ast::DefLocal(_, _) |
            ast::DefVariant(_, _, _) |
            ast::DefUpvar(_, _, _, _) => Some(recorder::VarRef),

            ast::DefFn(_, _) => Some(recorder::FnRef),
            ...
            ast::DefMethod(_, _) |
            ast::DefPrimTy(_) => {
                self.sess.bug(format!("lookup_def_kind for unexpected item: {:?}", def));
            },
        }
    }
    ...
    fn process_method(&mut self, method: &ast::Method, e:DxrVisitorEnv) {
        ...
        let qualname = qualname.into_owned();
    ...
    fn process_fn(&mut self,
        ...
        let qualname = self.analysis.ty_cx.map.path_to_str(item.id).into_owned();
        ...
        self.process_formals(&decl.inputs, qualname.clone(), e);
                      ...
                      expr: @ast::Expr)
    {
        ...
        let value = match mt {
            ast::MutMutable => "<mutable>".to_owned(),
            ast::MutImmutable => self.span.snippet(expr.span).into_owned(),
        };
    ...
    fn process_enum(&mut self,
        ...
        for variant in enum_definition.variants.iter() {
            ...
            let name = name.get();
            let qualname = qualname.clone().append("::").append(name).into_owned();
            let val = self.span.snippet(variant.span).into_owned();
        ...
        self.process_generic_params(ty_params, item.span, qualname.to_owned(), item.id, e);
    }
    ...
    fn process_path(&mut self,
        ...
        match *def {
            ...
            ast::DefBinding(id, _) => self.fmt.ref_str(recorder::VarRef,
                                                       ex.span,
                                                       sub_span,
                                                       ast_util::local_def(id),
                                                       e.cur_scope),
            ast::DefStatic(def_id,_) |
            ast::DefVariant(_, def_id, _) => self.fmt.ref_str(recorder::VarRef,
                                                              ex.span,
                                                              sub_span,
                                                              def_id,
                                                              e.cur_scope),
            ast::DefStruct(def_id) => self.fmt.ref_str(recorder::StructRef,
                                                       ex.span,
                                                       sub_span,
                                                       def_id,
                                                        e.cur_scope),
...
impl<'l> Visitor<DxrVisitorEnv> for DxrVisitor<'l> {
    fn visit_item(&mut self, item:&ast::Item, e: DxrVisitorEnv) {
        ...
        match item.node {
            ...
            ast::ItemTy(ty, ref ty_params) => {
                let qualname = self.analysis.ty_cx.map.path_to_str(item.id).into_owned();
                let value = ty_to_str(ty).into_owned();
                    ...
                    ast::ViewPathList(ref path, ref list, _) => {
                        for plid in list.iter() {
                            match self.lookup_type_ref(plid.node.id) {
                                Some(id) => match self.lookup_def_kind(plid.node.id) {
                                    Some(kind) => self.fmt.ref_str(kind,
                                                                   plid.span,
                                                                   Some(plid.span),
                                                                   id,
    ...
    fn visit_expr(&mut self, ex: &ast::Expr, e: DxrVisitorEnv) {
        ...
        match ex.node {
            ...
            ast::ExprFnBlock(decl, body) => {
                ...
                self.process_formals(&decl.inputs, "$" + ex.id.to_str(), e);
    ...
    fn visit_local(&mut self, l:&ast::Local, e: DxrVisitorEnv) {
        ...
        for &(id, ref p, ref immut, _) in self.collected_paths.iter() {
            let value = if *immut { value.to_owned() } else { "".to_owned() };
...
pub fn process_crate(sess: &Session,
    ...
    let (cratename, crateid) = match attr::find_crateid(krate.attrs.as_slice()) {
        ...
        None => {
            ...
            (StrBuf::from_str("unknown_crate"),"unknown_crate".to_owned())
        },
    };
    ...
    let output_file = match File::create(&root_path) {
        ...
        Err(_) => {
            let disp = root_path.display();
            sess.bug(format!("Could not open {}", disp));
        }
    };
    root_path.pop();
