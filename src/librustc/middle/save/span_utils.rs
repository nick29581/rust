...
impl<'a> SpanUtils<'a> {
    ...
    pub fn extent_str(&self, span: Span) -> ~str {
    ...
    pub fn snippet(&self, span: Span) -> StrBuf {
    ...
    pub fn retokenise_span(&self, span: Span) -> StringReader<'a> {
        ...
        let filemap = self.sess.codemap().new_filemap(StrBuf::from_str("<anon-dxr>"),
                                                      self.snippet(span));
        let s = self.sess;
        lexer::new_string_reader(s.diagnostic(), filemap)
    }
